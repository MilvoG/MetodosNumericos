...
print("")
print("Seja bem vindo ao metodo de multiplos passos!")
...
#vetor que vai guardar os primeiros 3 valores da funcao, calculadas pelo metodo de runge kutta
vetorF = []
...
for i in range (0, n):
    ...
    if (i <= 2):
        ...
        #adicionar valor obtido da funcao no vetorF para utilizarmos no metodo de multiplos passos
        vetorF.append(k1)

    else:

        #condicao para mostrar onde comeca os calculos por multiplos passos
        if (i == 3):
            print("Deste ponto em diante, todos os calculos sao feitos atraves do metodo de passos multiplos:\n")

        #precisamos calcular mais um F para podermos utilizar o metodo de multiplos passos
        #utilizamos k1 pois k1 = f(tn,yn)
        k1 = 1-t+(4*y)

        #adicionamos o ultimo valor de f(tn,yn) ao vetor
        vetorF.append(k1)

        #funcao para calcular todos yn >= 4 por multiplos passos
        yfinal = y + ((h/24)*((55*vetorF[-1])-(59*vetorF[-2])+(37*vetorF[-3])-(9*vetorF[-4])))

        #subtituimos as variaveis para continuar os calculos
        t += h
        y = yfinal

        #mostramos os valores de yn para todo yn >= 4
